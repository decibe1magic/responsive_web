;(function($) {
	...
	$.fn.scrolledFix = function(options) {
		...
		// 기본옵션 설정
		$.fn.scrolledFix.defaults = {	// 플러그인의 초기 옵션 defaults 설정
			scrolled_point : $(this).offset().top // scrolled_point에 플러그인이 적용된 요소의 화면 위에서부터의 높이를 참조
		}; 
		...
		return this.each(function() {
			...
			// 플러그인 적용 대상 참조.
			var $this = $(this);
				$win = $(win); //refInit() 함수가 반환한 win을 $()로 감싸 $win 변수에 참조시킨다.
				cloned = false,
				$gnb_ul = $('#gnb').find('ul');
				
				$this.data({ // 플러그인 적용 대상의 현재 설정 값을 data()에 저장한다.
					'pos' : $this.css('position'),
					'top' : $this.css('top'),
					'left': $this.css('left')
				});

				// 문서가 로드될 때, 스크롤 될 때 scrollFix 함수 호출
				$win.bind('laod scroll', scrolledFix); // 'load scroll' 이벤트 핸들러로 scrollFix 함수를 연결한다.
